...
def compute_ewma_vol(close: pd.DataFrame, cfg: Config, seed_window: int = 60, trading_days: int = 252):
    ...
    for ticker in close.columns:
        ...
        sigma = float(np.sqrt(var * trading_days))
        sigmas[ticker] = float(np.clip(sigma, cfg.vol_floor, cfg.vol_cap))
    return pd.Series(sigmas, name="sigma")


def compute_covariance(close: pd.DataFrame, cfg: Config, trading_days: int = 252):
...
def run_pipeline(csv_path, output_path, cfg: Config, basis="equity", force=False):
    ...
    trading_days = ql_risk.trading_days_per_year(datetime.now(timezone.utc).date())

    sigma = compute_ewma_vol(close[tickers], cfg, trading_days=trading_days)
    Sigma = compute_covariance(close[tickers], cfg, trading_days=trading_days)
